...
| 世界风格 | generator v11 仅以多格宽水带与镂刻海盆采样生成程序化 `sea`/`coastal` 地形 |
...
- 自动生成的多格宽六边格水网与镂刻海域、手工河道/湖泊 modifier、海岸、连续山体、地表纹理去重复、大气天空、城市、单位和战争迷雾。
- Worker 地形/植被生成、可选 IndexedDB 基础区块缓存，以及稀疏持久化格子覆盖。
- 分页数据驱动小地图，包含有界 Canvas/LRU 缓存、平滑缩放、后台概览生成和明确的目的地确认。
...
<http://127.0.0.1:3000/infinite-water.html>；其中的曲线和海盆几何与 generator v11
采样的包入口完全相同，使用与生产生成一致的边界函数。
...
| `three-hex-map/infinite-water-curve-field` | 供生产生成器与视觉检查页共用、可按范围查询的确定性曲线/折线与镂刻海盆场 |
...
- [世界风格生成 v1](docs/world-style-generation-v1.md) 保留 generator v6 冻结基线，并记录 generator v11 仅采样水体规则。
