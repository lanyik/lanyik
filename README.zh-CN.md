...
| 可玩游戏 | **余烬要塞**：基于无限世界的实时移动要塞求生，包含矿车运输、炮塔布防、定点炮击、虫潮突袭和完整战场存档 |
| 基础设施 | v1 已冻结；生命周期、所有权、调度、持久化和资源预算合同已有自动化验证 |
| 世界风格 | 生成 v1 现包含大尺度连通海域、确定性粗网格汇流河网、气候雪线、连续地貌和区域森林 |
| 玩法方向 | 迭代移动要塞求生玩法，游戏规则保持在冻结的运行时与地表合同之外 |
...
想直接玩游戏，打开 [**余烬要塞**](http://127.0.0.1:3000/emberwake.html)，
也可以点击演示页面的同名入口。驾驶移动要塞向东突围 48 格，派出矿车、部署炮塔，
在虫潮和吞噬带逼近时带着补给撤离。轨道炮击与主车共用燃料，可以随时战术暂停，
突破后也能继续深入无限世界；战场自动保存。完整规则与实现对应关系见
[游戏设计文档](docs/emberwake.md)。

打开[基建实验室](http://127.0.0.1:3000)，控制面板提供三种世界模式：
