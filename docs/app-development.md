...
状态：应用架构与开发合同；独立入口、会话与固定时钟，以及三类矿藏、地表勘察和起步
区域筛选已实现。建筑、采集和工业生产尚未实现。已落地行为见
[应用入口](./game/application-shell.md)与[矿藏勘察](./game/mineral-survey.md)。
玩法目标、首版范围和内容方向以 [游戏想法](../游戏想法.md)为准；本文定义代码组织、
权威状态、结算顺序和基建接入方式。下文的经营模块、结算阶段和存档接入仍为设计。
...
apps/expedition/
  ...
  src/
    app/
      ...
      GameSession.ts              # 已实现：会话、时间命令、加载边界；存档待接入
      WorldView.ts                # 已实现：地图加载与选中数据边界
    core/
      GameClock.ts                # 已实现：固定时钟、暂停与倍率
      resources/MineralField.ts   # 已实现：确定性初始矿藏；开采增量待接入
      ...
      ports/                      # 核心所需的地形、路线等接口
    content/                      # 已有 minerals.ts；建筑、配方、科技待接入
    scenarios/                    # 已有 landingSurvey.ts；后期场景约束待接入
    adapters/                     # 连接地图、导航和存档服务
    presentation/
      layers/                     # 已有 MineralLayer.ts；建筑、运输待接入
      interaction/                # 建造工具、选中与线路编辑
      App.tsx                     # 已实现：勘察面板与时间控制
      ui/                         # 待实现：经营管理面板
  tests/
...
| 矿藏 | core/resources 与 scenarios | 已有确定性矿点和初始储量；已开采量待采集模块接入 |
...
当前 `GameSession` 持有 `GameClock`、地图适配接口、只读勘察报告和选中快照，支持暂停、倍率和矿区定位命令；
`bootstrap.tsx` 持有 React、页面事件和动画循环。经营阶段扩展会话，接入
`IndustrySimulation`、业务命令、表现层和存档协调器。
...
初始步长为当前时钟采用的 100ms，实际工业执行开销仍需原型验证；所有参与经营的
区域使用同一步长。以下六阶段尚未实现，不能把时钟推进当成业务结算完成。
