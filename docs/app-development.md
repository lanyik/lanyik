...
指挥中心、采矿机、仓库、建造操作和直接入库已实现。已落地行为见
[应用入口](./game/application-shell.md)、[矿藏勘察](./game/mineral-survey.md)和
[基地建造与采矿](./game/construction-and-mining.md)。加工、电力、运输和游戏存档待实现。
...
apps/expedition/
  ...
  src/
    ...
    core/
      ...
      construction/Industry.ts    # 已实现：即时建造、拆除、基地服务与采矿
      inventory/InventoryLedger.ts # 已实现：容量、原子扣料与入库；预留待接入
      production/                 # 待实现：配方与加工批次
      power/                      # 连接图、发电、储能与负载
      ...
      ports/                      # 核心所需的地形、路线等接口
    content/                      # 已有 minerals.ts、buildings.ts；配方、科技待接入
...
| 生产批次 | 当前 Industry；后续 production | 已有采矿周期进度；配方、加工投入与输出缓存待实现 |
...
矿区定位、建造选择、旋转、放置和拆除命令。`bootstrap.tsx` 持有 React、页面事件、
B/R/Esc 快捷键和动画循环。先落地唯一指挥中心再推进时间，每次发布 tick 前完成实际
采集。后续多种工业作业出现时再拆出统一 `IndustrySimulation` 与存档协调器。
...
寻路。基地连通在布局改变时重算，正常 tick 按建筑创建顺序推进采矿和入库。
...
以下预留、分阶段施工和设备缓存属于后续加工系统的设计，不是当前采矿的前置条件：
...
当前只有一个基地共享库存。采矿机与新增仓库要求在指挥中心外缘 32 步的可达范围内，
路径受地形和完整建筑占地约束；堵路使已有采矿机停工，拆除后重新检查。实际搬运时间、
...
当前建造与采集状态只存在于会话内，重新勘察或刷新会开始新游戏。加工产线阶段接入
存档：应用实现一个必需的 `game-state` 参与者，连同
...
| 首条加工产线 | 冶炼、配方加工与一致性保存 | 批次投入和产出守恒，镜头外继续生产，半批次恢复一致 |
