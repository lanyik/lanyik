...
`;
  ...
  function resolveTile(numericSeed, profile, x, y, sampleAt, riverAt) {
    ...
    let type = !isWater2(sample.baseTerrain) && riverAt(x, y) ? "sea" /* sea */ : sample.baseTerrain;
    if (type === "sea" /* sea */) {
      const touchesLand = getNeighbors(x, y).some((neighbor) => {
        const adjacent = sampleAt(neighbor.x, neighbor.y);
        return adjacent !== void 0 && !isWater2(adjacent.baseTerrain) && !riverAt(neighbor.x, neighbor.y);
      });
  ...
  var PathFinder = class {
    ...
    find(startX, startY, endX, endY) {
      ...
      const startKey = pointKey2(start);
      const endKey = pointKey2(end);
      ...
      while (frontier.size > 0) {
        ...
        if (!current) break;
        const currentKey = pointKey2(current);
        ...
        for (const neighbor of getMapNeighbors(this.map, current.x, current.y)) {
          ...
          const neighborKey = pointKey2(neighbor);
